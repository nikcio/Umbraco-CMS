    ...
    where TEntity : class, IEntity
{
    ...
    ///     This method will not support passing in more than 2000 group IDs when also passing in entity IDs.
    /// </remarks>
    public async Task<EntityPermissionCollection> GetPermissionsForEntitiesAsync(int[] groupIds, CancellationToken? cancellationToken = null, params int[] entityIds)
    {
        var result = new EntityPermissionCollection();
        ...
        if (entityIds.Length == 0)
        {
            foreach (IEnumerable<int> group in groupIds.InGroupsOf(Constants.Sql.MaxParameterCount))
            {
                ...
                List<UserGroup2NodePermissionDto> permissions =
                    await AmbientScope.Database.FetchAsync<UserGroup2NodePermissionDto>(sql);
                foreach (EntityPermission permission in ConvertToPermissionList(permissions))
                {
                    result.Add(permission);
                }
    ...
    /// <param name="entityIds"></param>
    /// <returns></returns>
    public async Task<IEnumerable<EntityPermission>> GetPermissionsForEntitiesAsync(int[] entityIds, CancellationToken? cancellationToken = null)
    {
        Sql<ISqlContext> sql = Sql()
            .SelectAll()
            .From<UserGroup2NodeDto>()
        ...
        List<UserGroup2NodePermissionDto> result = await AmbientScope.Database.FetchAsync<UserGroup2NodePermissionDto>(sql);
        return ConvertToPermissionList(result);
    }
    ...
    ///     This will first clear the permissions for this user and entities and recreate them
    /// </remarks>
    public async Task ReplacePermissionsAsync(int groupId, IEnumerable<char>? permissions, CancellationToken? cancellationToken = null, params int[] entityIds)
    {
        if (entityIds.Length == 0)
        {
            return;
        ...
        foreach (IEnumerable<int> group in entityIds.InGroupsOf(Constants.Sql.MaxParameterCount))
        {
            await db.ExecuteAsync("DELETE FROM umbracoUserGroup2Node WHERE userGroupId = @groupId AND nodeId in (@nodeIds)", new { groupId, nodeIds = group });

            await db.ExecuteAsync("DELETE FROM umbracoUserGroup2NodePermission WHERE userGroupId = @groupId AND nodeId in (@nodeIds)", new { groupId, nodeIds = group });
        }
        ...
        if (permissions is not null)
        {
            ...
            await db.BulkInsertRecordsAsync(toInsert, cancellationToken);
            await db.BulkInsertRecordsAsync(toInsertPermissions, cancellationToken);
        }
    }
    ...
    /// <param name="permission"></param>
    /// <param name="entityIds"></param>
    public async Task AssignPermissionAsync(int groupId, char permission, CancellationToken? cancellationToken = null, params int[] entityIds)
    {
        IUmbracoDatabase db = AmbientScope.Database;

        await db.ExecuteAsync("DELETE FROM umbracoUserGroup2Node WHERE userGroupId = @groupId AND nodeId in (@entityIds)", new { groupId, entityIds });
        ...
        await db.BulkInsertRecordsAsync(actions, cancellationToken);
        await db.BulkInsertRecordsAsync(actionsPermissions, cancellationToken);
    }
    ...
    /// <param name="permission"></param>
    /// <param name="groupIds"></param>
    public async Task AssignEntityPermissionAsync(TEntity entity, char permission, IEnumerable<int> groupIds, CancellationToken? cancellationToken = null)
    {
        IUmbracoDatabase db = AmbientScope.Database;
        var groupIdsA = groupIds.ToArray();

        await db.ExecuteAsync("DELETE FROM umbracoUserGroup2Node WHERE nodeId = @nodeId AND userGroupId in (@groupIds)", new { nodeId = entity.Id, groupIds = groupIdsA });
        await db.ExecuteAsync(
            "DELETE FROM umbracoUserGroup2NodePermission WHERE nodeId = @nodeId AND permission = @permission AND userGroupId in (@groupIds)",
            new
            {
        ...
        await db.BulkInsertRecordsAsync(actions);
        await db.BulkInsertRecordsAsync(actionsPermissions);
    }
    ...
    ///     This will first clear the permissions for this entity then re-create them
    /// </remarks>
    public async Task ReplaceEntityPermissionsAsync(EntityPermissionSet permissionSet, CancellationToken? cancellationToken = null)
    {
        IUmbracoDatabase db = AmbientScope.Database;

        await db.ExecuteAsync("DELETE FROM umbracoUserGroup2Node WHERE nodeId = @nodeId", new { nodeId = permissionSet.EntityId });
        await db.ExecuteAsync("DELETE FROM umbracoUserGroup2NodePermission WHERE nodeId = @nodeId", new { nodeId = permissionSet.EntityId });
