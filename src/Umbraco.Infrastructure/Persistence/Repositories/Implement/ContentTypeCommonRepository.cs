...
internal class ContentTypeCommonRepository : IContentTypeCommonRepository
{
    ...
    public async Task<IEnumerable<IContentTypeComposition>?> GetAllTypesAsync(CancellationToken? cancellationToken = null)
    {
        // use a 5 minutes sliding cache - same as FullDataSet cache policy
        var cacheItem = await GetAllTypesInternalAsync(cancellationToken);
        return _appCaches.RuntimeCache.GetCacheItem(CacheKey, () => cacheItem, TimeSpan.FromMinutes(5), true);
    }
    ...
    private async Task<IEnumerable<IContentTypeComposition>> GetAllTypesInternalAsync(CancellationToken? cancellationToken = null)
    {
        if (Database == null)
        {
        ...
        List<ContentTypeDto>? contentTypeDtos = await Database.FetchAsync<ContentTypeDto>(sql1);
        ...
        List<ContentTypeAllowedContentTypeDto>? allowedDtos = await Database.FetchAsync<ContentTypeAllowedContentTypeDto>(sql2);
        ...
        await MapTemplatesAsync(contentTypes, cancellationToken);
        await MapCompositionAsync(contentTypes, cancellationToken);
        await MapGroupsAndPropertiesAsync(contentTypes, cancellationToken);
        await MapHistoryCleanupAsync(contentTypes, cancellationToken);
    ...
    private async Task MapHistoryCleanupAsync(Dictionary<int, IContentTypeComposition> contentTypes, CancellationToken? cancellationToken = null)
    {
        if (Database == null)
        {
            return;
        ...
        List<ContentVersionCleanupPolicyDto>? contentVersionCleanupPolicyDtos =
            await Database.FetchAsync<ContentVersionCleanupPolicyDto>(sql1);
    ...
    private async Task MapTemplatesAsync(Dictionary<int, IContentTypeComposition> contentTypes, CancellationToken? cancellationToken = null)
    {
        if (Database == null)
        {
            return;
        ...
        List<ContentTypeTemplateDto>? templateDtos = await Database.FetchAsync<ContentTypeTemplateDto>(sql1);
    ...
    private async Task MapCompositionAsync(IDictionary<int, IContentTypeComposition> contentTypes, CancellationToken? cancellationToken = null)
    {
        if (Database == null)
        {
            return;
        ...
        List<ContentType2ContentTypeDto>? compositionDtos = await Database.FetchAsync<ContentType2ContentTypeDto>(sql1);
    ...
    private async Task MapGroupsAndPropertiesAsync(IDictionary<int, IContentTypeComposition> contentTypes, CancellationToken? cancellationToken = null)
    {
        if (Database == null)
        {
            return;
        ...
        List<PropertyTypeGroupDto>? groupDtos = await Database.FetchAsync<PropertyTypeGroupDto>(sql1);
        ...
        List<PropertyTypeCommonDto>? propertyDtos = await Database.FetchAsync<PropertyTypeCommonDto>(sql2);
        Dictionary<string, PropertyType> builtinProperties =
            ConventionsHelper.GetStandardPropertyTypeStubs(_shortStringHelper);
