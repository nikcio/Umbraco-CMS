    ...
    where TEntity : class, IContentTypeComposition
{
    ...
    public async Task<TEntity?> GetAsync(Guid id, CancellationToken? cancellationToken = null) => await PerformGetAsync(id, cancellationToken);
    ...
    public async Task<IEnumerable<TEntity>> GetManyAsync(CancellationToken? cancellationToken = null, params Guid[]? ids) =>
        (await PerformGetAllAsync(cancellationToken, ids)) ?? Enumerable.Empty<TEntity>();
    ...
    public async Task<bool> ExistsAsync(Guid id, CancellationToken? cancellationToken = null) => await PerformExistsAsync(id, cancellationToken);
    ...
    public async Task<IEnumerable<MoveEventInfo<TEntity>>> MoveAsync(TEntity moving, EntityContainer? container, CancellationToken? cancellationToken = null)
    {
        var parentId = Constants.System.Root;
        if (container != null)
        {
        ...
        // update all descendants, update in order of level
        IEnumerable<TEntity> descendants = await GetAsync(Query<TEntity>().Where(type => type.Path.StartsWith(movingPath)), cancellationToken);
        var paths = new Dictionary<int, string>
        {
            [moving.Id] = moving.Path,
        };
    ...
    /// <param name="alias"></param>
    /// <returns></returns>
    public async Task<TEntity?> GetAsync(string alias, CancellationToken? cancellationToken = null) => await PerformGetAsync(alias, cancellationToken);
    ...
    protected override async Task<IEnumerable<TEntity>> PerformGetAllAsync(CancellationToken? cancellationToken = null, params int[]? ids)
    {
        IEnumerable<TEntity>? result = await GetAllWithFullCachePolicyAsync(cancellationToken);
    ...
    protected abstract Task<IEnumerable<TEntity>?> GetAllWithFullCachePolicyAsync(CancellationToken? cancellationToken = null);
    ...
    protected override async Task PersistDeletedItemAsync(TEntity entity, CancellationToken? cancellationToken = null)
    {
...
AND umbracoNode.nodeObjectType = @objectType",
    ...
    protected async Task PersistNewBaseContentTypeAsync(IContentTypeComposition entity, CancellationToken? cancellationToken = null)
    {
        ValidateVariations(entity);
...
AND umbracoNode.nodeObjectType = @objectType",
        ...
        nodeDto.Level = short.Parse(level.ToString(CultureInfo.InvariantCulture));
        nodeDto.SortOrder = sortOrder;
        var o = (await Database.IsNewAsync(nodeDto))
            ? Convert.ToInt32(Database.Insert(nodeDto))
            : Database.Update(nodeDto);
        ...
        foreach (IContentTypeComposition composition in entity.ContentTypeComposition)
        {
            ...
            if (composition.HasIdentity)
            {
                await Database.InsertAsync(new ContentType2ContentTypeDto { ParentId = composition.Id, ChildId = entity.Id });
            }
            else
            {
                // Fallback for ContentTypes with no identity
                ContentTypeDto? contentTypeDto =
                    await Database.FirstOrDefaultAsync<ContentTypeDto>(
                        "WHERE alias = @Alias",
                        new { composition.Alias });
                if (contentTypeDto != null)
                {
                    await Database.InsertAsync(new ContentType2ContentTypeDto
                    {
                        ParentId = contentTypeDto.NodeId,
                        ChildId = entity.Id,
                    });
        ...
        if (entity.AllowedContentTypes is not null)
        {
            ...
            foreach (ContentTypeSort allowedContentType in entity.AllowedContentTypes)
            {
                await Database.InsertAsync(new ContentTypeAllowedContentTypeDto
                {
                    Id = entity.Id,
                    AllowedId = allowedContentType.Id.Value,
                    SortOrder = allowedContentType.SortOrder,
        ...
        {
            PropertyTypeGroupDto tabDto = PropertyGroupFactory.BuildGroupDto(propertyGroup, nodeDto.NodeId);
            var primaryKey = Convert.ToInt32(await Database.InsertAsync(tabDto));
            propertyGroup.Id = primaryKey; // Set Id on PropertyGroup
        ...
        foreach (IPropertyType propertyType in entity.PropertyTypes)
        {
            ...
            // Update the current PropertyType with correct PropertyEditorAlias and DatabaseType
            DataTypeDto? dataTypeDto =
                await Database.FirstOrDefaultAsync<DataTypeDto>("WHERE nodeId = @Id", new { Id = propertyTypeDto.DataTypeId });
            propertyType.PropertyEditorAlias = dataTypeDto.EditorAlias;
            propertyType.ValueStorageType = dataTypeDto.DbType.EnumParse<ValueStorageType>(true);
        }
...
AND umbracoNode.id <> @id",
    ...
    protected async Task PersistUpdatedBaseContentTypeAsync(IContentTypeComposition entity, CancellationToken? cancellationToken = null)
    {
        CorrectPropertyTypeVariations(entity);
        ValidateVariations(entity);
...
AND umbracoNode.id <> @id",
        ...
        // not carry it and therefore the dto does not have it yet - must get it from db,
        // look up ContentType entry to get PrimaryKey for updating the DTO
        ContentTypeDto? dtoPk = await Database.FirstAsync<ContentTypeDto>("WHERE nodeId = @Id", new { entity.Id });
        dto.PrimaryKey = dtoPk.PrimaryKey;
        ...
        if (entity.RemovedContentTypes.Any())
        {
            ...
            Sql<ISqlContext> sql = Sql()
                ...
                .Where<NodeDto>(x => x.NodeObjectType == Constants.ObjectTypes.Document)
                .Where<ContentDto>(x => x.ContentTypeId == entity.Id);
            List<ContentDto>? contentDtos = await Database.FetchAsync<ContentDto>(sql);
            ...
            foreach (var key in entity.RemovedContentTypes)
            {
                // find PropertyTypes for the removed ContentType
                List<PropertyTypeDto>? propertyTypes =
                    await Database.FetchAsync<PropertyTypeDto>("WHERE contentTypeId = @Id", new { Id = key });
        ...
        {
            List<PropertyTypeDto>? dbPropertyTypes =
                await Database.FetchAsync<PropertyTypeDto>("WHERE contentTypeId = @Id", new { entity.Id });
            IEnumerable<int> dbPropertyTypeIds = dbPropertyTypes.Select(x => x.Id);
            IEnumerable<int> entityPropertyTypes = entity.PropertyTypes.Where(x => x.HasIdentity).Select(x => x.Id);
            IEnumerable<int> propertyTypeToDeleteIds = dbPropertyTypeIds.Except(entityPropertyTypes);
            foreach (var propertyTypeId in propertyTypeToDeleteIds)
            {
                await DeletePropertyTypeAsync(entity.Id, propertyTypeId, cancellationToken);
            }
        }
        ...
        if (entity.IsPropertyDirty("PropertyGroups"))
        {
            ...
            // get the tabs that are currently existing (in the db), get the tabs that we want,
            // now, and derive the tabs that we want to delete
            var existingPropertyGroups = (await Database
                .FetchAsync<PropertyTypeGroupDto>("WHERE contentTypeNodeId = @id", new { id = entity.Id }))
                .Select(x => x.Id)
                .ToList();
            var newPropertyGroups = entity.PropertyGroups.Select(x => x.Id).ToList();
            var groupsToDelete = existingPropertyGroups
            ...
            if (groupsToDelete.Length > 0)
            {
                ...
                // - keep track of them, later on we'll figure out what to do with them
                // see http://issues.umbraco.org/issue/U4-8663
                orphanPropertyTypeIds = (await Database.FetchAsync<PropertyTypeDto>(
                    "WHERE propertyTypeGroupId IN (@ids)",
                    new { ids = groupsToDelete }))
                    .Select(x => x.Id).ToList();
                Database.Update<PropertyTypeDto>(
        ...
        foreach (PropertyGroup propertyGroup in entity.PropertyGroups)
        {
            ...
            PropertyTypeGroupDto groupDto = PropertyGroupFactory.BuildGroupDto(propertyGroup, entity.Id);
            var groupId = propertyGroup.HasIdentity
                ? await Database.UpdateAsync(groupDto)
                : Convert.ToInt32(await Database.InsertAsync(groupDto));
            if (propertyGroup.HasIdentity == false)
            {
                propertyGroup.Id = groupId;
            }
        ...
        if (contentTypeVariationChanging)
        {
            await MoveContentTypeVariantDataAsync(entity, oldContentTypeVariation, newContentTypeVariation, cancellationToken);
            await Clear301RedirectsAsync(entity, cancellationToken);
            await ClearScheduledPublishingAsync(entity, cancellationToken);
        }
        ...
        Dictionary<int, (ContentVariation FromVariation, ContentVariation ToVariation)>? propertyTypeVariationChanges =
            propertyTypeVariationDirty != null
                ? await GetPropertyVariationChangesAsync(propertyTypeVariationDirty, cancellationToken)
                : null;
        ...
        foreach (IPropertyType propertyType in entity.PropertyTypes)
        {
            ...
            if (propertyType.DataTypeId == 0 || propertyType.DataTypeId == default)
            {
                await AssignDataTypeFromPropertyEditorAsync(propertyType, cancellationToken);
            }
            ...
            PropertyTypeDto propertyTypeDto =
                PropertyGroupFactory.BuildPropertyTypeDto(groupId, propertyType, entity.Id);
            var typeId = propertyType.HasIdentity
                ? await Database.UpdateAsync(propertyTypeDto)
                : Convert.ToInt32(Database.Insert(propertyTypeDto));
            if (propertyType.HasIdentity == false)
            {
                propertyType.Id = typeId;
        ...
        // getting 'all' from the cache policy is prone to race conditions - fast but dangerous
        // var all = ((FullDataSetRepositoryCachePolicy<TEntity, int>)CachePolicy).GetAllCached(PerformGetAll);
        IEnumerable<TEntity>? all = await PerformGetAllAsync(cancellationToken);
        ...
        if (propertyTypeVariationChanges?.Count > 0)
        {
            await MovePropertyTypeVariantDataAsync(propertyTypeVariationChanges, impacted, cancellationToken);
        }
        ...
        if (orphanPropertyTypeIds != null)
        {
            foreach (var id in orphanPropertyTypeIds)
            {
                await DeletePropertyTypeAsync(entity.Id, id, cancellationToken);
            }
        }
    ...
    // gets property types that have actually changed, and the corresponding changes
    // returns null if no property type has actually changed
    private Task<Dictionary<int, (ContentVariation FromVariation, ContentVariation ToVariation)>?>
        GetPropertyVariationChangesAsync(IEnumerable<IPropertyType> propertyTypes, CancellationToken? cancellationToken = null)
    {
        var propertyTypesL = propertyTypes.ToList();
    ...
    ///     Clear any redirects associated with content for a content type
    /// </summary>
    private async Task Clear301RedirectsAsync(IContentTypeComposition contentType, CancellationToken? cancellationToken = null)
    {
        // first clear out any existing property data that might already exists under the default lang
        Sql<ISqlContext> sqlSelect = Sql().Select<NodeDto>(x => x.UniqueId)
            .From<NodeDto>()
        ...
        await Database.ExecuteAsync(sqlDelete);
    }
    ...
    ///     Clear any scheduled publishing associated with content for a content type
    /// </summary>
    private async Task ClearScheduledPublishingAsync(IContentTypeComposition contentType, CancellationToken? cancellationToken = null)
    {
        // TODO: Fill this in when scheduled publishing is enabled for variants
    }
    ...
    ///     Gets the default language identifier.
    /// </summary>
    private async Task<int> GetDefaultLanguageIdAsync(CancellationToken? cancellationToken = null)
    {
        Sql<ISqlContext> selectDefaultLanguageId = Sql()
            .Select<LanguageDto>(x => x.Id)
            .From<LanguageDto>()
        ...
        return await Database.FirstAsync<int>(selectDefaultLanguageId);
    }
    ...
    private async Task MovePropertyTypeVariantDataAsync(
        IDictionary<int, (ContentVariation FromVariation, ContentVariation ToVariation)> propertyTypeChanges,
        IEnumerable<IContentTypeComposition> impacted, CancellationToken? cancellationToken = null)
    {
                 ...
                 propertyTypeChanges.GroupBy(x => x.Value))
        {
            ...
            {
                // Culture has been enabled
                await CopyPropertyDataAsync(null, defaultLanguageId, propertyTypeIds, impactedL, cancellationToken);
                await CopyTagDataAsync(null, defaultLanguageId, propertyTypeIds, impactedL, cancellationToken);
                await RenormalizeDocumentEditedFlagsAsync(propertyTypeIds, impactedL, cancellationToken);
            }
            else if (fromCultureEnabled && !toCultureEnabled)
            {
                // Culture has been disabled
                await CopyPropertyDataAsync(defaultLanguageId, null, propertyTypeIds, impactedL, cancellationToken);
                await CopyTagDataAsync(defaultLanguageId, null, propertyTypeIds, impactedL, cancellationToken);
                await RenormalizeDocumentEditedFlagsAsync(propertyTypeIds, impactedL, cancellationToken);
            }
        }
    }
    ...
    ///     Moves variant data for a content type variation change.
    /// </summary>
    private async Task MoveContentTypeVariantDataAsync(IContentTypeComposition contentType, ContentVariation fromVariation,
        ContentVariation toVariation, CancellationToken? cancellationToken = null)
    {
        var defaultLanguageId = await GetDefaultLanguageIdAsync(cancellationToken);
        ...
        if (cultureIsNotEnabled && cultureWillBeEnabled)
        {
            ...
            await Database.ExecuteAsync(sqlInsert);
    ...
    private async Task CopyTagDataAsync(
        int? sourceLanguageId,
        int? targetLanguageId,
        IReadOnlyCollection<int> propertyTypeIds,
        IReadOnlyCollection<int>? contentTypeIds = null,
        CancellationToken? cancellationToken = null)
    {
        ...
        await Database.ExecuteAsync(sqlDeleteRelations);
        ...
        */
        ...
        await Database.ExecuteAsync(sqlInsertTags);
        ...
        await Database.ExecuteAsync(sqlInsertRelations);
    ...
    /// <param name="propertyTypeIds">The property type identifiers.</param>
    /// <param name="contentTypeIds">The content type identifiers.</param>
    private async Task CopyPropertyDataAsync(int? sourceLanguageId, int? targetLanguageId,
        IReadOnlyCollection<int> propertyTypeIds, IReadOnlyCollection<int>? contentTypeIds = null, CancellationToken? cancellationToken = null)
    {
        // note: important to use SqlNullableEquals for nullable types, cannot directly compare language identifiers
        var whereInArgsCount = propertyTypeIds.Count + (contentTypeIds?.Count ?? 0);
        if (whereInArgsCount > Constants.Sql.MaxParameterCount)
    ...
    private async Task RenormalizeDocumentEditedFlagsAsync(
        ...
        CancellationToken? cancellationToken = null)
    {
        var defaultLang = await LanguageRepository.GetDefaultIdAsync(cancellationToken);
        ...
        foreach (IGrouping<bool, DocumentCultureVariationDto> editValue in toUpdate.GroupBy(x => x.Edited))
        {
            await Database.ExecuteAsync(Sql().Update<DocumentCultureVariationDto>(u => u.Set(x => x.Edited, editValue.Key))
                .WhereIn<DocumentCultureVariationDto>(x => x.Id, editValue.Select(x => x.Id)));
        }
        ...
        foreach (IGrouping<bool, KeyValuePair<int, bool>> editValue in editedDocument.GroupBy(x => x.Value))
        {
            await Database.ExecuteAsync(Sql().Update<DocumentDto>(u => u.Set(x => x.Edited, editValue.Key))
                .WhereIn<DocumentDto>(x => x.NodeId, editValue.Select(x => x.Key)));
        }
    }
    ...
    private Task DeletePropertyTypeAsync(int contentTypeId, int propertyTypeId, CancellationToken? cancellationToken = null)
    {
        // first clear dependencies
        Database.Delete<TagRelationshipDto>("WHERE propertyTypeId = @Id", new { Id = propertyTypeId });
        Database.Delete<PropertyDataDto>("WHERE propertyTypeId = @Id", new { Id = propertyTypeId });
    ...
    protected abstract Task<TEntity?> PerformGetAsync(Guid id, CancellationToken? cancellationToken = null);
    ...
    /// </summary>
    /// <param name="propertyType"></param>
    private async Task AssignDataTypeFromPropertyEditorAsync(IPropertyType propertyType, CancellationToken? cancellationToken = null)
    {
        // we cannot try to assign a data type of it's empty
        if (propertyType.PropertyEditorAlias.IsNullOrWhiteSpace() == false)
        {
            Sql<ISqlContext> sql = Sql()
                ...
                .Where(
                    ...
                    new { propertyEditorAlias = propertyType.PropertyEditorAlias })
                .OrderBy<DataTypeDto>(typeDto => typeDto.NodeId);
            DataTypeDto? datatype = await Database.FirstOrDefaultAsync<DataTypeDto>(sql);
    ...
    protected abstract Task<TEntity?> PerformGetAsync(string alias, CancellationToken? cancellationToken = null);

    protected abstract IEnumerable<TEntity>? PerformGetAll(params Guid[]? ids);

    protected abstract Task<IEnumerable<TEntity>?> PerformGetAllAsync(CancellationToken? cancellationToken = null, params Guid[]? ids);

    protected abstract bool PerformExists(Guid id);

    protected abstract Task<bool> PerformExistsAsync(Guid id, CancellationToken? cancellationToken = null);
...
WHERE cmsContentType." + aliasColumn + @" LIKE @pattern",
    ...
    public async Task<string> GetUniqueAliasAsync(string alias, CancellationToken? cancellationToken = null)
    {
        // alias is unique across ALL content types!
        var aliasColumn = SqlSyntax.GetQuotedColumnName("alias");
        List<string>? aliases = await Database.FetchAsync<string>(
            @"SELECT cmsContentType." + aliasColumn + @" FROM cmsContentType
INNER JOIN umbracoNode ON cmsContentType.nodeId = umbracoNode.id
WHERE cmsContentType." + aliasColumn + @" LIKE @pattern",
            new { pattern = alias + "%", objectType = NodeObjectTypeId });
    ...
    public async Task<bool> HasContainerInPathAsync(string contentPath, CancellationToken? cancellationToken = null)
    {
        var ids = contentPath.Split(Constants.CharArrays.Comma)
            .Select(s => int.Parse(s, CultureInfo.InvariantCulture)).ToArray();
        return await HasContainerInPathAsync(cancellationToken, ids);
    }
...
WHERE {Constants.DatabaseSchema.Tables.Content}.nodeId IN (@ids) AND cmsContentType.isContainer=@isContainer",
    ...
    public async Task<bool> HasContainerInPathAsync(CancellationToken? cancellationToken = null, params int[] ids)
    {
        var sql = new Sql(
            $@"SELECT COUNT(*) FROM cmsContentType
INNER JOIN {Constants.DatabaseSchema.Tables.Content} ON cmsContentType.nodeId={Constants.DatabaseSchema.Tables.Content}.contentTypeId
WHERE {Constants.DatabaseSchema.Tables.Content}.nodeId IN (@ids) AND cmsContentType.isContainer=@isContainer",
    ...
    ///     Returns true or false depending on whether content nodes have been created based on the provided content type id.
    /// </summary>
    public async Task<bool> HasContentNodesAsync(int id, CancellationToken? cancellationToken = null)
    {
        var sql = new Sql(
            $"SELECT CASE WHEN EXISTS (SELECT * FROM {Constants.DatabaseSchema.Tables.Content} WHERE contentTypeId = @id) THEN 1 ELSE 0 END",
            new { id });
        return await Database.ExecuteScalarAsync<int>(sql) == 1;
    }
